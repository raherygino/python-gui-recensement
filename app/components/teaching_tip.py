...
from qfluentwidgets import FluentIconBase, isDarkTheme, FlyoutView, FlyoutViewBase
...
class TopTailTeachingTipManager(TeachingTipManager):
    ...
    def _pos(self, tip: TeachingTip):
        if type(tip.target).__name__ == "QPoint":
            x = tip.target.x() - tip.sizeHint().width() // 2
            y = tip.target.y() - tip.layout().contentsMargins().top()
            return QPoint(x, y)
        else:
            target = tip.target
